Shader "Teraria/Planet Surface"
{
    Properties
    {
        ...
        _GlobalColorTint("Global Planet Tint", Color) = (1, 1, 1, 1)
        _GlobalSaturation("Global Saturation", Range(0,2)) = 1
        _GlobalContrast("Global Contrast", Range(0,2)) = 1
        _ColdClimateTint("Cold Climate Tint", Color) = (0.72, 0.82, 1, 1)
        _WarmClimateTint("Warm Climate Tint", Color) = (1, 0.86, 0.58, 1)
        _WetClimateTint("Wet Climate Tint", Color) = (0.62, 0.82, 0.7, 1)
        _DryClimateTint("Dry Climate Tint", Color) = (0.88, 0.72, 0.46, 1)
        _SnowTint("Snow Tint", Color) = (0.9, 0.94, 1, 1)
        _SnowStartHeight("Snow Start Height", Range(0,1)) = 0.72
        _SnowBlend("Snow Blend", Range(0,1)) = 0.18
        _WetnessStrength("Moisture Wetness", Range(0,1)) = 0.45
        _Season("Season Phase", Range(0,1)) = 0
        _SeasonStrength("Season Strength", Range(0,1)) = 0
        _SpringTint("Spring Seasonal Tint", Color) = (0.72, 1, 0.68, 1)
        _AutumnTint("Autumn Seasonal Tint", Color) = (1, 0.58, 0.28, 1)
        _FarDetailStart("Far Detail Start", Float) = 120
        _FarDetailEnd("Far Detail End", Float) = 420
        _FarDetailTint("Far Detail Tint", Color) = (0.48, 0.52, 0.55, 1)
    ...
    SubShader
    {
        ...
        fixed4 _ShoreTint;
        fixed4 _RiverTint;
        fixed4 _GlobalColorTint;
        fixed4 _ColdClimateTint;
        fixed4 _WarmClimateTint;
        fixed4 _WetClimateTint;
        fixed4 _DryClimateTint;
        fixed4 _SnowTint;
        fixed4 _SpringTint;
        fixed4 _AutumnTint;
        fixed4 _FarDetailTint;
        ...
        half _GlobalContrast;
        half _SnowStartHeight;
        half _SnowBlend;
        half _WetnessStrength;
        half _Season;
        half _SeasonStrength;
        half _FarDetailStart;
        half _FarDetailEnd;
        half _DebugClimate;
        ...
        fixed3 ApplyPlanetColorCorrections(fixed3 color)
        {
            fixed luminance = dot(color, fixed3(0.2126, 0.7152, 0.0722));
            color = lerp(fixed3(luminance, luminance, luminance), color, _GlobalSaturation);
            color = (color - 0.5) * _GlobalContrast + 0.5;
            return saturate(color * _GlobalColorTint.rgb);
        }

        fixed3 SeasonalTint(half season)
        {
            half spring = saturate(1.0 - abs(season - 0.15) * 4.0);
            half autumn = saturate(1.0 - abs(season - 0.65) * 4.0);
            fixed3 tint = lerp(fixed3(1, 1, 1), _SpringTint.rgb, spring * 0.35);
            return lerp(tint, _AutumnTint.rgb, autumn * 0.45);
        }
        ...
        void surf(Input IN, inout SurfaceOutputStandard o)
        {
            ...
            fixed3 temperatureTint = lerp(_ColdClimateTint.rgb, _WarmClimateTint.rgb, temperature);
            fixed3 moistureTint = lerp(_DryClimateTint.rgb, _WetClimateTint.rgb, moisture);
            biomeColor *= lerp(fixed3(1, 1, 1), temperatureTint * moistureTint, 0.28);

            half snowMask = smoothstep(_SnowStartHeight, saturate(_SnowStartHeight + _SnowBlend), heightMask) * saturate((0.55 - temperature) * 2.4) * (1.0 - riverMask);
            biomeColor = lerp(biomeColor, _SnowTint.rgb, snowMask);
            biomeColor *= lerp(fixed3(1, 1, 1), SeasonalTint(_Season), _SeasonStrength);

            half distanceToCamera = distance(_WorldSpaceCameraPos, IN.worldPos);
            half farMask = smoothstep(_FarDetailStart, _FarDetailEnd, distanceToCamera);
            biomeColor = lerp(biomeColor, _FarDetailTint.rgb * dot(biomeColor, fixed3(0.333, 0.333, 0.333)), farMask * 0.28);
            biomeColor = ApplyPlanetColorCorrections(biomeColor);

            fixed3 climateDebug = fixed3(temperature, moisture, heightMask);
            o.Albedo = lerp(biomeColor, climateDebug, _DebugClimate);
            o.Smoothness = saturate(_BiomeSmoothness0 * weights.x + _BiomeSmoothness1 * weights.y + _BiomeSmoothness2 * weights.z + _BiomeSmoothness3 * weights.w + rockMask * 0.15 + coastMask * 0.18 + moisture * _WetnessStrength * 0.35);
            o.Metallic = 0;
        }
        ENDCG
    }
    FallBack "Teraria/Biome Terrain"
}
